...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void exchangeEntries(T[] array, int i, int j) {
        ...
        //Only proceeds if array indices are not the same
        if (i != j) {
            T temp = array[i];
            array[i] = array[j];
            array[j] = temp;
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        //Checks existence of left tree
        int leftTree = 2 * top + 1;
        if (leftTree <= last) {

            //Checks existence of right tree and compares with left tree
            int lowerIndex = leftTree;
            int rightTree = 2 * top + 2;
            if (rightTree <= last
                    && order.compare(array[rightTree], array[leftTree]) < 0) {

                lowerIndex = rightTree;
            }

            //If top is bigger than the lower-value tree, swap the values
            if (order.compare(array[top], array[lowerIndex]) > 0) {

                exchangeEntries(array, lowerIndex, top);
                siftDown(array, lowerIndex, last, order);
            }
        }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        //Checks if left and right trees are heaps. If they are not, then heapify them
        if (!isHeap(array, leftIndex, array.length - 1, order)) {

            heapify(array, leftIndex, order);
        }
        if (!isHeap(array, rightIndex, array.length - 1, order)) {

            heapify(array, rightIndex, order);
        }

        //Once they are both heaps, sift down the top index
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        //Assigns all elements from queue into heap array, then heapifies
        for (int i = 0; i < heap.length; i++) {
            T element = q.dequeue();
            heap[i] = element;
        }
        heapify(heap, top, order);
    ...
    public final void changeToExtractionMode() {
        ...
        /*
         * Builds heap from queue and stores into this.heap, initializes
         * heapSize, changes to extraction mode
         */
    ...
    public final T removeFirst() {
        ...
        //First, extracts the top index value
        int top = 0;
        T first = this.heap[top];

        /*
         * Exchange entries with the last index and top, then I sift down the
         * new top index
         */
        exchangeEntries(this.heap, top, this.heapSize - 1);
        this.heapSize--;
    ...
    public final int size() {
        /*
         * If this is in insertion mode, use queue length. If this is in
         * extraction mode, use heapSize.
         */
        int length = 0;
        //If in insertion mode, then queue length else heap length
        if (this.insertionMode) {

            length = this.entries.length();
        } else {

            length = this.heapSize;
